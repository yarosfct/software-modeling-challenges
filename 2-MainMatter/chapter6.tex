...
Support needs that participants articulated therefore cluster around (i)~seeing a mapping from text to model, (ii)~getting a correctness signal while the artefact can still change, and (iii)~getting that signal in a format they will actually use (desk walk-around versus raising a hand; private versus public). Research on teaching with modelling tools similarly argues for feedback written for learners, not the cryptic error messages of an industrial compiler \cite{liebel2017model}. This study adds the access dimension: the same walk-around that helps a reserved student is not the same as a public board that an outgoing student enjoys, and neither is the same as comments that arrive after the grade is locked.

CAT13 should not be over-read as an AI-education finding. Four student cases, including one Portuguese incident, reject generation for graded work. That aligns with PH3 (checks and explanations, not auto-drawn homework). It does not evaluate any particular tool. L3's wish for an LLM that explains a student's model back, rather than writing it, is an implication (Section~\ref{sec:disc_implications}), not part of CAT13.
...
%
% A later study could address the remaining questions without pretending this MSc can still collect that sample:
%
% \begin{itemize}
%     \item Observe the first minutes of a board lab, or interview the lecturer of the same offering as the students, to test wait-to-start as a room-level versus person-level claim.
%     \item Interview a lecturer who teaches multi-notation AMS or RE specifically about whether a bridge method is in the syllabus (OQ4 remainder).
%     \item Collect non-leading incidents about choosing granularity and what to omit, or observe the same task, before treating abstraction deficit as a student finding (OQ5).
%     \item Trial a private in-lab validator with reserved students if the research question is about CAT6 recovery---as a design study, not as an extension of these interviews.
% \end{itemize}
%
% Global saturation across countries, notations, and lecturer practice remains out of reach for a master's timeline; it is a different project.
